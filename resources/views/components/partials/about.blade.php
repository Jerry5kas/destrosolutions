@php
    // Get services grouped by subtitle from the controller
    $servicesBySubtitle = $servicesBySubtitle ?? collect();
    $serviceCategories = $servicesBySubtitle->keys()->filter()->values();
    $firstCategory = $serviceCategories->first();
@endphp
...
<div data-reveal-scope>
    ...
    <div class="about-tabs-container text-xs xs:text-sm text-center max-w-xs xs:max-w-sm sm:max-w-2xl md:max-w-4xl lg:max-w-6xl xl:max-w-7xl mx-auto px-3 xs:px-4 sm:px-6">
        {{-- Dynamic tab headers based on service categories --}}
        <div
            ...
            @foreach($serviceCategories as $index => $category)
                ...
                >
                    ...
                    @elseif(str_contains(strtolower($category), 'railway') || str_contains(strtolower($category), 'train'))
                        <svg fill="" version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 612 612" xml:space="preserve" class="h-6 w-6 sm:h-7 sm:w-7 md:h-8 md:w-8 transition-all duration-300">
                    ...
                    @elseif(str_contains(strtolower($category), 'health') || str_contains(strtolower($category), 'medical'))
                        <svg class="h-6 w-6 sm:h-7 sm:w-7 md:h-8 md:w-8 transition-all duration-300" fill="currentColor" version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 256 190" enable-background="new 0 0 256 190" xml:space="preserve">
                            <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                            <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
                            <g id="SVGRepo_iconCarrier">
                    ...
                    @else
                        <!-- Default gear icon for other categories -->
                        <svg viewBox="0 0 20 20" class="h-6 w-6 sm:h-7 sm:w-7 md:h-8 md:w-8 transition-all duration-300" xmlns="http://www.w3.org/2000/svg" fill="currentColor">
                            <path fill-rule="evenodd" d="M8.03774 0.858582C8.10805 0.365936 8.52993 0 9.02749 0H11.2931C11.7907 0 12.2125 0.365936 12.2829 0.858582L12.6542 3.45728C13.3566 3.72517 14.0048 4.10292 14.5778 4.56953L17.0155 3.591C17.4774 3.40564 18.0053 3.58806 18.254 4.01904L19.3869 5.98094C19.6356 6.41193 19.5297 6.96028 19.1383 7.26755L17.0727 8.88898C17.1115 9.13266 17.1378 9.38052 17.1508 9.6319C17.1571 9.7538 17.1603 9.87653 17.1603 10C17.1603 10.3781 17.1303 10.7492 17.0727 11.111L19.1383 12.7324C19.5297 13.0397 19.6356 13.5881 19.3869 14.019L18.254 15.9809C18.0053 16.4119 17.4774 16.5943 17.0155 16.409L14.5778 15.4305C14.0048 15.8971 13.3566 16.2748 12.6542 16.5427L12.2829 19.1414C12.2125 19.6341 11.7907 20 11.2931 20H9.02749C8.52993 20 8.10805 19.6341 8.03774 19.1414L7.6664 16.5427C6.96401 16.2748 6.31582 15.897 5.74282 15.4304L3.30483 16.409C2.84292 16.5943 2.31509 16.4119 2.06631 15.9809L0.933738 14.019C0.684714 13.5881 0.790671 13.0397 1.18227 12.7324L3.24795 11.1109C3.19033 10.7491 3.1603 10.3781 3.1603 10C3.1603 9.62193 3.19033 9.25085 3.24795 8.88904L1.18227 7.26755C0.790671 6.96028 0.684714 6.41193 0.933738 5.98094L2.06631 4.01904C2.31509 3.58806 2.84292 3.40564 3.30483 3.591L5.74282 4.5696C6.31582 4.10295 6.96401 3.72517 7.6664 3.45728L8.03774 0.858582ZM8.77735 6.51823L13.376 9.58397C13.6728 9.78189 13.6728 10.2181 13.376 10.416L8.77735 13.4818C8.44507 13.7033 8 13.4651 8 13.0657V10V6.93426C8 6.53491 8.44507 6.29672 8.77735 6.51823Z"/>
                        </svg>
                    @endif

                    <!-- Dynamic Label with proper text wrapping -->
                    <div class="relative text-xs sm:text-sm font-medium {{ strlen($category) > 12 ? 'text-center leading-tight' : 'whitespace-nowrap' }} max-w-20 sm:max-w-24 md:max-w-28 lg:max-w-32">
                        {{ $category }}
                        <!-- Animated underline -->
                        <span class="tab-underline absolute left-0 -bottom-1 h-0.5 transition-all duration-300 ease-out origin-left"></span>
                    </div>
                </div>
            @endforeach
        </div>

        {{-- Dynamic tab content based on service categories --}}
        <div class="mt-4 xs:mt-5 sm:mt-6">
            @foreach($serviceCategories as $index => $category)
                @php
                    $tabId = \Illuminate\Support\Str::slug($category);
                    $isFirst = $index === 0;
                    $services = $servicesBySubtitle->get($category, collect());
                @endphp
                <div 
                    data-tab-content="{{ $tabId }}" 
                    class="tab-content max-w-xs xs:max-w-sm sm:max-w-2xl md:max-w-4xl lg:max-w-6xl xl:max-w-7xl mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 xs:gap-4 sm:gap-5 md:gap-6 lg:gap-8 xl:gap-10 2xl:gap-16 px-2 xs:px-3 sm:px-4 {{ $isFirst ? 'block' : 'hidden' }}"
                    style="{{ $isFirst ? 'display: grid; opacity: 1;' : 'display: none; opacity: 0;' }}"
                >
                    @forelse($services as $service)
                        <x-partials.card1 
                            :src="$service->image ? Storage::url($service->image) : asset('images/car1.jpg')"
                            :title="$service->title"
                            :desc="$service->description ? Str::limit($service->description, 80) : 'Professional service solution'"
                        />
                    @empty
...
document.addEventListener('DOMContentLoaded', function() {
    console.log('Initializing Dynamic About Tabs System');

    const tabButtons = document.querySelectorAll('.tab-button');
    const tabContents = document.querySelectorAll('.tab-content');
    
    // Get the first tab as default active tab
    let activeTab = tabButtons.length > 0 ? tabButtons[0].getAttribute('data-tab') : null;

    // Initialize tabs
    function initTabs() {
        console.log('Setting up dynamic tab system');
    ...
    function switchTab(tabName) {
        ...
        console.log(`Switching from ${activeTab} to ${tabName}`);
    ...
    // Update tab display (buttons and content)
    function updateTabDisplay() {
        console.log(`Updating display for active tab: ${activeTab}`);

        // Update tab buttons
        tabButtons.forEach(button => {
            const tabName = button.getAttribute('data-tab');
            const icon = button.querySelector('svg');
            const text = button.querySelector('.relative');
            const underline = button.querySelector('.tab-underline');

            if (tabName === activeTab) {
                // Active state - Blue theme
                button.classList.add('active');
                if (icon) icon.style.color = '#2563eb'; // blue-600
                if (text) text.style.color = '#2563eb'; // blue-600
                if (underline) {
                    underline.style.backgroundColor = '#2563eb'; // blue-600
                ...
                if (icon) icon.style.color = '#475569'; // slate-600
                if (text) text.style.color = '#475569'; // slate-600
                if (underline) {
                    underline.style.backgroundColor = '#475569'; // slate-600
    ...
    function animateTabTransition() {
        ...
        console.log('Animating tab content transition');

        // Force reflow
        activeContent.offsetHeight;

        // Animate in with smooth transition
        activeContent.style.transition = 'opacity 0.4s cubic-bezier(0.4, 0, 0.2, 1), transform 0.4s cubic-bezier(0.4, 0, 0.2, 1)';
        activeContent.style.opacity = '1';
        activeContent.style.transform = 'translateY(0)';

        // Animate cards with improved stagger
        const cards = activeContent.querySelectorAll('[data-reveal]');
        cards.forEach((card, index) => {
            ...
            }, index * 80 + 150); // Faster stagger for smoother feel
        });
    }

    // Initialize the tab system with a small delay to ensure DOM is ready
    setTimeout(() => {
        initTabs();
        
        // Trigger initial animation for first tab content
        if (activeTab) {
            setTimeout(() => {
                animateTabTransition();
            }, 100);
    ...
    console.log('Dynamic About Tabs System initialized successfully');
});
</script>
...
/* Dynamic text wrapping for tab labels */
.tab-button .relative {
...
/* Smooth tab switching performance */
.tab-button {
...
.about-tabs-container .overflow-x-auto::-webkit-scrollbar {
    height: 4px;
}

.about-tabs-container .overflow-x-auto::-webkit-scrollbar-track {
    background: #f1f5f9;
    border-radius: 2px;
}

.about-tabs-container .overflow-x-auto::-webkit-scrollbar-thumb {
    background: #cbd5e1;
    border-radius: 2px;
}

.about-tabs-container .overflow-x-auto::-webkit-scrollbar-thumb:hover {
    background: #94a3b8;
}
</style>
