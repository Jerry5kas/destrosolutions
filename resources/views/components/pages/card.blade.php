        ...
        <div>
            ...
            <ul class="space-y-2 text-sm text-gray-700 text-xs">
                @if($item && isset($item->key_features) && count($item->key_features) > 0)
                    @foreach(array_slice($item->key_features, 0, 2) as $feature)
                        <li class="flex items-start">
                            <svg class="w-3.5 h-3.5 mt-0.5 mr-3 text-blue-600" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7" />
                            </svg>
                            <span>{{ Str::limit($feature, 50) }}</span>
                        </li>
                    @endforeach
                ...
                @else
                    <li class="flex items-start">
                        ...
                        <span>Default feature 1</span>
                    </li>
                    <li class="flex items-start">
                        <svg class="w-3.5 h-3.5 mt-0.5 mr-3 text-blue-600" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7" />
                        </svg>
                        <span>Default feature 2</span>
                    </li>
                @endif
            </ul>
        </div>
