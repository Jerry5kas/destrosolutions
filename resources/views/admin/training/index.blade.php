<x-admin-layout :title="'Training'" :header="'Training'">
    <div x-data="{ 
        ...
        <div class="flex justify-between items-center mb-6">
            <div>
                <h2 class="text-xl font-semibold text-gray-800">Training</h2>
                <p class="text-sm text-gray-600 mt-1">Manage your training content</p>
            </div>
            <a href="{{ route('admin.training.create') }}" class="btn-primary flex items-center gap-2">
        ...
        <div x-show="!loading" x-transition>
                                ...
                                @foreach($training as $index => $item)
                                    <tr class="hover:bg-gray-50 {{ $index % 2 === 0 ? 'bg-white' : 'bg-gray-50/50' }}" 
                                        ...
                                        <td class="px-6 py-4">
                                            ...
                                            @if($item->objectives && count($item->objectives) > 0)
                                                <div class="text-sm text-gray-500">{{ count($item->objectives) }} objectives</div>
                                            @endif
                                        </td>
                                        <td class="px-6 py-4">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                            <div class="flex items-center justify-end gap-2" x-show="showActions" x-transition>
                                                ...
                                                <form method="POST" action="{{ route('admin.training.destroy', $item) }}" class="inline" onsubmit="return confirm('Are you sure you want to delete this training content?')">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit" 
            ...
            @else
                <div class="text-center py-12">
                    ...
                    <h3 class="text-lg font-medium text-gray-900 mb-2">No training content found</h3>
                    <p class="text-gray-500 mb-6">Get started by creating your first training content to educate your audience.</p>
                    <a href="{{ route('admin.training.create') }}" class="btn-primary flex items-center gap-2">
                        <x-icons name="plus" size="w-4 h-4" />
                        Create New Training
                    </a>
