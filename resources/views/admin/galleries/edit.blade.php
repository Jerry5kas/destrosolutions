<x-admin-layout :title="'Edit Gallery'" :header="'Edit Gallery'">
    <form method="POST" action="{{ route('admin.galleries.update', $gallery) }}" enctype="multipart/form-data" class="bg-white border border-gray-200 rounded-xl p-6 space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <input name="title" value="{{ old('title', $gallery->title) }}" class="input-field {{ $errors->has('title') ? 'input-error' : '' }}" placeholder="Enter gallery title" required />
                @error('title')
            ...
            <div>
                ...
                <input type="number" name="sort_order" value="{{ old('sort_order', $gallery->sort_order) }}" min="0" class="input-field {{ $errors->has('sort_order') ? 'input-error' : '' }}" placeholder="Display order" />
                @error('sort_order')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
        ...
        <div>
            ...
            <textarea name="description" rows="4" class="textarea-field {{ $errors->has('description') ? 'input-error' : '' }}" placeholder="Enter gallery description...">{{ old('description', $gallery->description) }}</textarea>
            @error('description')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div>
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <div class="relative">
                    <img src="{{ Storage::url($image) }}" class="w-full h-24 object-cover rounded-lg border border-gray-200" />
        ...
        <div class="flex items-center">
            <input type="checkbox" name="is_active" value="1" id="is_active" class="h-4 w-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500" {{ old('is_active', $gallery->is_active) ? 'checked' : '' }}>
