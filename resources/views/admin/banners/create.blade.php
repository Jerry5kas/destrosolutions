<x-admin-layout :title="'Create Banner'" :header="'Create Banner'">
    <form method="POST" action="{{ route('admin.banners.store') }}" enctype="multipart/form-data" class="glass-card rounded-xl p-6 space-y-6">
        @csrf
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                <label class="block text-sm font-medium text-white/90 mb-2">Title <span class="text-red-400">*</span></label>
                <input name="title" value="{{ old('title') }}" class="glass-input w-full rounded-lg px-4 py-3 {{ $errors->has('title') ? 'border-red-400/50' : '' }}" placeholder="Enter banner title" required />
                @error('title')
            ...
            <div>
                ...
                <input name="slogan" value="{{ old('slogan') }}" class="glass-input w-full rounded-lg px-4 py-3" placeholder="Enter banner slogan" />
            </div>
            <div>
        ...
        <div>
            ...
            <textarea name="description" rows="4" class="glass-input w-full rounded-lg px-4 py-3 {{ $errors->has('description') ? 'border-red-400/50' : '' }}" placeholder="Enter banner description...">{{ old('description') }}</textarea>
            @error('description')
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
                ...
                <input name="text1" value="{{ old('text1') }}" class="glass-input w-full rounded-lg px-4 py-3" placeholder="Enter text 1" />
            </div>
            <div>
                <label class="block text-sm font-medium text-white/90 mb-2">Text 2</label>
                <input name="text2" value="{{ old('text2') }}" class="glass-input w-full rounded-lg px-4 py-3" placeholder="Enter text 2" />
            </div>
            <div>
                <label class="block text-sm font-medium text-white/90 mb-2">Text 3</label>
                <input name="text3" value="{{ old('text3') }}" class="glass-input w-full rounded-lg px-4 py-3" placeholder="Enter text 3" />
            </div>
        </div>
        <div class="flex items-center">
            <input type="checkbox" name="is_active" value="1" id="is_active" class="h-4 w-4 rounded border-white/30 text-blue-400 focus:ring-blue-500 bg-white/10" {{ old('is_active', true) ? 'checked' : '' }}>
