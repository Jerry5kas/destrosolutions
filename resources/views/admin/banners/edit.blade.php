<x-admin-layout :title="'Edit Banner'" :header="'Edit Banner'">
    <form method="POST" action="{{ route('admin.banners.update', $banner) }}" enctype="multipart/form-data" class="bg-white border border-gray-200 rounded-xl p-6 space-y-6">
        @csrf
        @method('PUT')
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">Title <span class="text-red-500">*</span></label>
                <input name="title" value="{{ old('title', $banner->title) }}" class="input-field {{ $errors->has('title') ? 'input-error' : '' }}" placeholder="Enter banner title" required />
                @error('title')
            ...
            <div>
                ...
                <input name="slogan" value="{{ old('slogan', $banner->slogan) }}" class="input-field" placeholder="Enter banner slogan" />
            </div>
            <div>
        ...
        <div>
            ...
            <textarea name="description" rows="4" class="textarea-field {{ $errors->has('description') ? 'input-error' : '' }}" placeholder="Enter banner description...">{{ old('description', $banner->description) }}</textarea>
            @error('description')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            @enderror
        </div>
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
                ...
                <input name="text1" value="{{ old('text1', $banner->text1) }}" class="input-field" placeholder="Enter text 1" />
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">Text 2</label>
                <input name="text2" value="{{ old('text2', $banner->text2) }}" class="input-field" placeholder="Enter text 2" />
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">Text 3</label>
                <input name="text3" value="{{ old('text3', $banner->text3) }}" class="input-field" placeholder="Enter text 3" />
            </div>
        </div>
        <div class="flex items-center">
            <input type="checkbox" name="is_active" value="1" id="is_active" class="h-4 w-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500" {{ old('is_active', $banner->is_active) ? 'checked' : '' }}>
            <label for="is_active" class="ml-2 text-sm font-medium text-gray-700">Active</label>
        </div>
        <div class="flex items-center gap-3 pt-4">
            <button type="submit" class="btn-primary" x-data="{ loading: false }" @click="loading = true" :disabled="loading">
                <span x-show="!loading">Update Banner</span>
