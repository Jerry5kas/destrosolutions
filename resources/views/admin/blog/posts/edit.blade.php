...
<x-admin-layout :title="'Edit Blog Post'" :header="'Edit Blog Post'">
    <form method="POST" action="{{ route('admin.blog.posts.update', $post) }}" enctype="multipart/form-data" class="bg-white border border-gray-200 rounded-xl p-6 space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="flex items-center">
                <input type="checkbox" name="is_featured" value="1" id="is_featured" class="h-4 w-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500" {{ old('is_featured', $post->is_featured) ? 'checked' : '' }}>
        ...
        <div>
            ...
            <textarea name="description" rows="4" class="textarea-field {{ $errors->has('description') ? 'input-error' : '' }}" placeholder="Enter a short description/excerpt..." required>{{ old('description', $post->description) }}</textarea>
            @error('description')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div>
            ...
            @php
                $contentValue = old('subcontent');
                if (!$contentValue && $post->subcontent) {
                    // subcontent should be stored as JSON string in DB
                    $contentValue = is_string($post->subcontent) ? $post->subcontent : json_encode($post->subcontent);
                }
            @endphp
            <input type="hidden" name="subcontent" id="subcontent-input" value="{{ $contentValue ?? '' }}" />
            <div id="subcontent-editor" class="bg-white {{ $errors->has('subcontent') ? 'border-2 border-red-500' : '' }}" data-content="{{ $contentValue ?? '' }}"></div>
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                    @foreach($post->images as $image)
                        <div class="border border-gray-200 rounded-lg p-2">
                            <img src="{{ Storage::url($image->image) }}" class="w-full h-24 object-cover rounded" />
                            <p class="text-xs text-gray-600 mt-2 truncate">{{ $image->alt ?? 'No alt text' }}</p>
                        </div>
                    @endforeach
                </div>
            </div>
        ...
        <div x-data="{ imageCount: 1 }">
            ...
            <p class="text-xs text-gray-500 mb-3">Add additional images to the gallery.</p>
