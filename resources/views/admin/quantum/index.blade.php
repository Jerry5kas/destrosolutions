<x-admin-layout :title="'Quantum'" :header="'Quantum'">
    <div x-data="{ 
        ...
        <div class="flex justify-between items-center mb-6">
            <div>
                <h2 class="text-xl font-semibold text-gray-800">Quantum</h2>
                <p class="text-sm text-gray-600 mt-1">Manage your quantum content</p>
            </div>
            <a href="{{ route('admin.quantum.create') }}" class="btn-primary flex items-center gap-2">
        ...
        <div x-show="!loading" x-transition>
                                    ...
                                    <tr class="hover:bg-gray-50 {{ $index % 2 === 0 ? 'bg-white' : 'bg-gray-50/50' }}" 
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                            <div class="flex items-center justify-end gap-2" x-show="showActions" x-transition>
                                                <a href="{{ route('admin.quantum.edit', $item) }}" 
                                                ...
                                                <form method="POST" action="{{ route('admin.quantum.destroy', $item) }}" class="inline" onsubmit="return confirm('Are you sure you want to delete this quantum content?')">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit" 
            ...
            @else
                <div class="text-center py-12">
                    ...
                    <h3 class="text-lg font-medium text-gray-900 mb-2">No quantum content found</h3>
                    <p class="text-gray-500 mb-6">Get started by creating your first quantum content to showcase your services.</p>
                    <a href="{{ route('admin.quantum.create') }}" class="btn-primary flex items-center gap-2">
                        <x-icons name="plus" size="w-4 h-4" />
                        Create New Quantum Content
                    </a>
                </div>
            @endif
        </div>
