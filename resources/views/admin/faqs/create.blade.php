<x-admin-layout :title="'Create FAQ'" :header="'Create FAQ'">
    <form method="POST" action="{{ route('admin.faqs.store') }}" class="bg-white border border-gray-200 rounded-xl p-6 space-y-6">
        @csrf
        
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">Question <span class="text-red-500">*</span></label>
                <input name="question" value="{{ old('question') }}" class="input-field {{ $errors->has('question') ? 'input-error' : '' }}" placeholder="Enter FAQ question" required />
                @error('question')
        ...
        <div>
            ...
            <textarea name="answer" rows="6" class="textarea-field {{ $errors->has('answer') ? 'input-error' : '' }}" placeholder="Enter detailed answer..." required>{{ old('answer') }}</textarea>
            @error('answer')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            @enderror
        </div>
